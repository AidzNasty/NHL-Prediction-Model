...
/* Selectbox selected value — target baseweb select directly */
        ...
        for _, pred in preds.iterrows():
            ...
            with st.container(border=True):
                ...
                with c2:
                    ...
                    st.metric("Goal Diff / GP", f"{xgf:+.2f}", delta_color=color)
                with c3:
                    color = "normal" if gsax > 0 else "inverse"
                    st.metric("Elo Edge", f"{gsax*100:+.0f}", delta_color=color)
    ...
    with tab1:
        ...
        stats = [
            ...
            ("xGF% — Expected Goals For Percentage",
             ...
             "This is the most predictive of the possession stats. A team can out-Corsi their opponent but still lose in xGF% if their shots come from bad angles. xGF% above 50% signals genuine shot-quality dominance. (Shown here as team context — the game model uses point-in-time Elo and goal-based form rather than season-aggregate xGF%.)"),
    ...
    with tab2:
        ...
        stats2 = [
            ...
            ("GSAX — Goals Saved Above Expected",
             ...
             "Positive GSAX = goalie outperformed expectations; negative = underperformed. A full-season GSAX of +10 or more is an elite performance. (Shown here as team context — per-game goalie data isn't captured yet, so the game model doesn't use GSAX directly.)"),
    ...
    with tab3:
        ...
        stats3 = [
            ("Goal Differential / Game",
             "The difference between the two teams' season-to-date goal differential per game, computed point-in-time (only games before the matchup).",
             "Goal Diff/GP = Home (GF-GA)/game - Away (GF-GA)/game",
             "Positive = home team has outscored opponents by more this season. One of the highest-weight features in the model."),

            ("Elo Edge",
             "The gap in the two teams' Elo ratings. Elo updates game-by-game (with home-ice and margin-of-victory) and carries across seasons, so it is fully leak-free.",
             "Elo Edge = Home Elo - Away Elo (before home-ice adjustment)",
             "Positive = home team is the stronger side by Elo. Elo and its win probability are the model's most important features."),

            ("HomeIce Differential",
             "A composite home-ice advantage score combining the home team's point-in-time home win rate and the away team's road win rate.",
             "HomeIce Diff = (Home home-W% - Away road-W%) x 6",
